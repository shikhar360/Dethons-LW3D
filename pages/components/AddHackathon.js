...
export default function AddHackathon({handleActive}) {
  ...
  const { mutateAsync: addHacks, isLoading } = useContractWrite(contract, "addHacks")

  const addHackathonTx = async (val) => {
    try {
      const _amount = ethers.utils.parseEther("0.02");
      const data = await addHacks([ val.name, val.startDate, val.regEnd, val.endDate, val.desc, +val.prize ,{value : _amount}]);
      setLoading(true)
