...
export default function EditHackathon({ closeEdit, prizex, descname, datex }) {
  // console.log(closeEdit, activeEdit);
  ...
  async function settingDates(val) {
    try {
      ...
      const dateTx = await contract.setDates(
        val.startDate,
        val.regEnd,
        val.endDate,
        { value: _amount }
  ...
  async function settingNameDesc(val) {
    try {
      const _amount = ethers.utils.parseEther("0.01");
      const ndTx = await contract.setDetails(val.name, val.desc, {
        value: _amount,
      });
      setLoading(true);
      await ndTx.wait();
  ...
  async function settingPrize(val) {
    try {
      const _amount = ethers.utils.parseEther("0.01");
      const prizeTx = await contract.increasePrize(val.prize, {
        value: _amount,
      });
      setLoading(true);
      await prizeTx.wait();
